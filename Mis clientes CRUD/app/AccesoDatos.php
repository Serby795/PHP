...
class AccesoDatos
{
    ...
    private function __construct()
    {
        ...
        $this->dbh = new mysqli(DB_SERVER, DB_USER, DB_PASSWORD, DB_NAME);
    ...
    function getNumClientes(): int {
        $resultado = $this->dbh->query("SELECT COUNT(*) FROM Clientes");
        $row = $resultado->fetch_row();
        return $row[0];
    }

    // SELECT Devuelvo la lista de Usuarios
    public function getUsuarios($desde, $limite): array
    {
        $tuser = [];
        // Crea la sentencia preparada
        $stmt_usuarios = $this->dbh->prepare("select * from clientes limit $desde, $limite");
